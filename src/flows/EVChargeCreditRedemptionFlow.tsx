...
export const EVChargeCreditRedemptionFlow: React.FC<Props> = ({ onClose }) => {
  ...
  const next = () => {
    if (step === 'confirm') {
      ...
      const request: RedemptionRequest = { id, creditId: selectedCredit.id, participantId: 'session-participant', chargingHubId: selectedHub.id, requestedUnits: selectedCredit.amount, unitLabel: 'Green Route Credits', status: 'requested', requestedAt: new Date().toISOString() };
  ...
  return <main className="charging-benefit-flow">
    <header className="charging-benefit-flow__header"><button type="button" onClick={previous} className="green-wallet-icon-button" aria-label="Go back"><ArrowLeft size={19} /></button><div><span>Green Route Wallet</span><strong>EV Charge Credit</strong></div><span className="charging-benefit-demo-pill">Research beta</span></header>
    ...
    <div className="charging-benefit-deck">
      ...
      <AnimatePresence mode="wait" initial={false}><motion.section key={step} className={`charging-benefit-card charging-benefit-card--${step}`} initial={reduceMotion ? false : { opacity: 0, rotateY: 12, x: 22, scale: 0.97 }} animate={{ opacity: 1, rotateY: 0, x: 0, scale: 1 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, rotateY: -10, x: -18, scale: 0.98 }} transition={{ type: 'spring', stiffness: 250, damping: 27 }}>
        {step === 'details' && <><div className="charging-benefit-card__icon"><BatteryCharging size={28} /></div><span className="green-wallet-kicker">Eligible program benefit</span><h1>Request an EV Charge Credit</h1><p className="charging-benefit-card__intro">Use approved Green Route Credits to request a manually reviewed benefit at an eligible Charging Hub.</p><div className="charging-benefit-facts"><div><span>Available credits</span><strong>{selectedCredit.amount}</strong></div><div><span>Request type</span><strong>Full credit</strong></div><div><span>Review</span><strong>Required</strong></div></div><div className="charging-benefit-safety-note"><ShieldCheck size={18} /><p>This request does not reserve a charger, start a session, or process a payment.</p></div></>}
