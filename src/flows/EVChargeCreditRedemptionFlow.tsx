...
export const EVChargeCreditRedemptionFlow: React.FC<Props> = ({ onClose }) => {
  ...
  const selectedCredit = credits.find(credit => credit.status === 'approved');
  ...
  const next = () => {
    if (step === 'confirm' && selectedCredit && selectedHub) {
      ...
      const request: RedemptionRequest = { id, creditId: selectedCredit.id, participantId: 'session-participant', chargingHubId: selectedHub.id, requestedUnits: selectedCredit?.amount ?? 0, unitLabel: 'kWh-equivalent', status: 'requested', requestedAt: new Date().toISOString() };
  ...
  return <main className="charging-benefit-flow">
    ...
    <div className="charging-benefit-deck">
      ...
      <AnimatePresence mode="wait" initial={false}><motion.section key={step} className={`charging-benefit-card charging-benefit-card--${step}`} initial={reduceMotion ? false : { opacity: 0, rotateY: 12, x: 22, scale: 0.97 }} animate={{ opacity: 1, rotateY: 0, x: 0, scale: 1 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, rotateY: -10, x: -18, scale: 0.98 }} transition={{ type: 'spring', stiffness: 250, damping: 27 }}>
        {step === 'details' && <><div className="charging-benefit-card__icon"><BatteryCharging size={28} /></div><span className="green-wallet-kicker">Eligible program benefit</span><h1>Request an EV Charge Benefit</h1><p className="charging-benefit-card__intro">Use an approved program benefit to request a manually reviewed kWh-equivalent allocation at an eligible Charging Hub.</p><div className="charging-benefit-facts"><div><span>Available benefit</span><strong>{selectedCredit?.amount ?? 0} kWh-equivalent</strong></div><div><span>Request type</span><strong>Full kWh-equivalent benefit</strong></div><div><span>Review</span><strong>Required</strong></div></div><div className="charging-benefit-safety-note"><ShieldCheck size={18} /><p>This request does not reserve a charger, start a session, or process a payment.</p></div></>}
        {step === 'location' && <><div className="charging-benefit-card__icon"><MapPin size={27} /></div><span className="green-wallet-kicker">Eligible Charging Hubs</span><h1>Choose a Charging Hub</h1><p className="charging-benefit-card__intro">Choose a program-configured location. Availability is not live and access remains subject to program rules.</p><div className="charging-location-list">{chargingHubs.length === 0 ? <div className="charging-benefit-empty-state"><strong>No eligible Charging Hubs are available</strong><small>Connect the Green Wallet API to load verified program locations.</small></div> : chargingHubs.map(hub => <button type="button" key={hub.id} className={`charging-location-card ${locationId === hub.id ? 'is-selected' : ''}`} onClick={() => setLocationId(hub.id)}><span className="charging-location-card__marker"><MapPin size={18} /></span><span><strong>{hub.name}</strong><small>{hub.city} · {hub.network} · {hub.status}</small></span><span className="charging-location-card__check"><BadgeCheck size={18} /></span></button>)}</div></>}
        {step === 'confirm' && <><div className="charging-benefit-card__icon"><TicketCheck size={27} /></div><span className="green-wallet-kicker">Review before submitting</span><h1>Confirm your request</h1><div className="charging-confirmation-list"><div><span>Benefit</span><strong>EV Charge Benefit</strong></div><div><span>kWh-equivalent requested</span><strong>{selectedCredit?.amount ?? 0} kWh-equivalent</strong></div><div><span>Charging Hub</span><strong>{selectedHub?.name ?? 'No Charging Hub selected'}</strong></div><div><span>Review</span><strong>Administrator required</strong></div></div><label className="charging-benefit-confirmation"><input type="checkbox" checked={acknowledged} onChange={event => setAcknowledged(event.target.checked)} /><span>I understand this is a request for review. It does not activate, reserve, or guarantee access to a charger.</span></label></>}
