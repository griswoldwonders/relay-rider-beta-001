...
export interface RouteSignal {
  id: string; corridor: string; startingArea: string; destinationArea: string; campusAffiliation: string;
  daysOfWeek: string[]; timeWindow: string; routeType: 'recurring' | 'occasional' | 'event' | 'medical' | 'campus' | 'other';
  relayZoneType: string[]; transitOptions: string[]; studentTransitPass: 'yes' | 'no' | 'not-sure'; incentiveInterests: string[];
  evPreference: 'ev-only' | 'hybrid-ev' | 'any'; maxWalkingDistance: string; privacyPreference: string;
  status: 'draft' | 'submitted' | 'in-research' | 'needs-signals' | 'needs-participant' | 'partner-review' | 'legal-review' | 'not-active';
  routeFit: 'high' | 'moderate' | 'low'; greenRouteCredit: number; createdAt: string;
  modeled?: { overlapPotential: 'high' | 'medium' | 'low'; timeCompatibility: 'strong' | 'moderate' | 'needs-data'; relayZoneFit: 'strong' | 'moderate' | 'needs-review'; evHybridSupply: 'available' | 'needs-more'; parkingPressure: 'low' | 'medium' | 'high'; pilotReadiness: 'research-only' | 'partner-review' | 'not-ready'; };
}

export interface EVParticipantSignal { id: string; vehicleType: 'ev' | 'phev' | 'hybrid' | 'other'; vehicleMake: string; vehicleModel: string; vehicleYear: string; startingArea: string; destinationArea: string; travelDays: string[]; timeWindow: string; maxDetour: string; relayZoneTypes: string[]; feedbackCallWilling: boolean; reviewsAccepted: string[]; status: 'submitted' | 'in-review' | 'approved' | 'rejected'; createdAt: string; }
