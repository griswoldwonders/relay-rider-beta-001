...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createRedemptionRequest = (request: RedemptionRequest) => setRedemptionRequests(current =>
    current.some(item => item.creditId === request.creditId && ['requested', 'under-review'].includes(item.status))
      ? current
      : [...current, request]
  );

  const reviewRedemptionRequest = (id: string, status: 'fulfilled' | 'denied', note: string) => setRedemptionRequests(current =>
    current.map(request => request.id === id
      ? { ...request, status, reviewNote: note, reviewedAt: new Date().toISOString(), reviewedBy: 'program-admin' }
      : request
    )
  );
