...
export const WalletScreen: React.FC<WalletScreenProps> = ({ onBack }) => {
  ...
  return (
    <div className="green-wallet-screen">
      <Header title="Green Wallet" subtitle="Institution-sponsored research-beta program benefit" />
      <div className="container green-wallet-shell">
        ...
        <section className="green-wallet-balance">
          <div className="green-wallet-balance__top"><span>Available research-beta benefit</span><small>Manual pilot</small></div>
          <strong>{available.toLocaleString()} Green Route Credits</strong>
          <p>Issued institution-sponsored benefits for eligible manual-pilot requests, not a cash balance.</p>
        </section>
        <div className="green-wallet-status-grid">
          <div className="green-wallet-status-card green-wallet-status-card--approved"><span>Available</span><strong>{available} credits</strong><small>issued research-beta benefit</small></div>
          <div className="green-wallet-status-card green-wallet-status-card--pending"><span>Under review</span><strong>{pending} credits</strong><small>manual program review in progress</small></div>
          <div className="green-wallet-status-card green-wallet-status-card--redeemed"><span>Redeemed</span><strong>{redeemed} credits</strong><small>manual program decision recorded</small></div>
        </div>
        ...
        <div className="green-wallet-benefit-list">
          <button type="button" className="green-wallet-benefit-card green-wallet-benefit-card--charging" onClick={() => setShowRedemption(true)} disabled={available === 0 || chargingHubs.filter(hub => hub.status === 'active').length === 0}>
            <span className="green-wallet-benefit-card__icon"><BatteryCharging size={23} /></span>
            <span className="green-wallet-benefit-card__copy"><small>Available for pilot review</small><strong>EV Charge Benefit</strong><span>Request administrative review of an eligible Green Route Credit at an active program-configured Charging Hub.</span></span>
            <span className="green-wallet-benefit-card__action">{available > 0 && chargingHubs.some(hub => hub.status === 'active') ? 'Explore' : 'Unavailable'} <ArrowRight size={17} /></span>
          </button>
        ...
        <div className="green-wallet-disclosure"><ShieldCheck size={15} className="mb-2" /><strong>Program disclosure</strong><br />Green Wallet is an institution-sponsored research-beta benefit. Green Route Credits are not cash, wages, fares, guaranteed payments, certified carbon credits, or automatic charging payments. There is no live availability feed, no reservation, and no automatic charging settlement.</div>
      </div>
    </div>
  );
};
