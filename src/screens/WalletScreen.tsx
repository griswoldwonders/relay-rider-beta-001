...
export const WalletScreen: React.FC<WalletScreenProps> = ({ onBack }) => {
  ...
  const available = useMemo(() => credits.filter(c => isAvailableCredit(c.status)).reduce((sum, c) => sum + creditUnits(c), 0), [credits]);
    ...
    <div className="green-wallet-screen">
      ...
      <div className="container green-wallet-shell">
        ...
        <div className="green-wallet-status-grid">
          ...
          <div className="green-wallet-status-card green-wallet-status-card--pending"><span>Under review</span><strong>{pending} credits</strong><small>benefit under review</small></div>
        ...
        <div className="green-wallet-activity-list">
          {credits.length === 0 ? <div className="green-wallet-empty-state"><strong>No program activity yet</strong><small>Connect the Green Wallet API to load participant credits and earning history.</small></div> : credits.map(credit => <div className="green-wallet-activity" key={credit.id}><div><strong>{credit.activity}</strong><small>{isAvailableCredit(credit.status) ? 'Available' : credit.status === 'pending' ? 'Under review' : credit.status === 'expired' ? 'Expired' : 'Redeemed'} • {creditIssuedAt(credit)}</small></div><span>{credit.status === 'redeemed' ? '−' : '+'}{creditUnits(credit)} {credit.unitLabel ?? 'Green Route Credits'}</span></div>)}
        </div>
