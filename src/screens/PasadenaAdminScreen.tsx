...
const classificationLabel = pasadenaInterventionAssessment.classification.replaceAll('_', ' ').toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <button type="button" onClick={onBack} aria-label="Back to Relay Rider home" className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white hover:bg-slate-50">
              <ArrowLeft size={18} />
            </button>
            <div>
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.35fr_0.65fr] lg:items-end">
            <div>
              <p className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-emerald-200">
                <Sparkles size={14} /> Clean commute opportunity
              </p>
              <h2 className="max-w-4xl text-3xl font-black tracking-tight sm:text-4xl">
                Identify which gasoline SOV trips are addressable, by which clean intervention, and with what evidence.
              </h2>
              <p className="mt-4 max-w-3xl text-sm leading-6 text-slate-300 sm:text-base">
                Relay Rider compares supported multimodal alternatives first, then applies deeper planned-BEV/PHEV compatibility and modeled net-VMT analysis where eligible.
              </p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
              ...
              <button type="button" onClick={() => setShowEvidence(value => !value)} className="mt-4 inline-flex items-center gap-2 font-semibold text-emerald-300 hover:text-emerald-200">
            ...
            {pasadenaMetrics.map(metric => (
              <article key={metric.label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <EvidenceBadge state={metric.evidence} />
                </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.88fr_1.12fr]">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Opportunity funnel</p><h2 className="font-bold">Gasoline SOV → high-fit EV preview</h2></div>
            </div>
            <div className="mt-5 space-y-3">
              {pasadenaFunnel.map((step, index) => {
                ...
                return (
                  <div key={step.label}>
                    <div className="mb-1 flex items-center justify-between text-xs"><span className="font-medium text-slate-600">{index + 1}. {step.label}</span><strong>{step.value}</strong></div>
                    <div className="h-2.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-emerald-600" style={{ width: `${width}%` }} /></div>
                  </div>
                );
              })}
            </div>
          ...
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="w-full min-w-[650px] text-left text-xs">
                <thead className="text-slate-500"><tr className="border-b border-slate-200"><th className="pb-2">Corridor</th><th className="pb-2">Gas SOV</th><th className="pb-2">EV routes</th><th className="pb-2">Seats</th><th className="pb-2">High-fit</th><th className="pb-2">Detour</th><th className="pb-2">Priority</th></tr></thead>
                <tbody>
                  {pasadenaCorridors.map(corridor => (
                    <tr key={corridor.id} onClick={() => setSelectedCorridor(corridor.id)} className={`cursor-pointer border-b border-slate-100 hover:bg-slate-50 ${selectedCorridor === corridor.id ? 'bg-emerald-50/70' : ''}`}>
                      <td className="py-3 pr-3 font-semibold">{corridor.name}</td><td>{corridor.gasolineSov}</td><td>{corridor.plannedEvRoutes}</td><td>{corridor.availableSeats}</td><td>{corridor.highFitPreviews}</td><td>{corridor.medianDetourMinutes.toFixed(1)} min</td><td><span className={corridor.priority === 'High' ? 'rounded-full bg-emerald-100 px-2 py-1 font-semibold text-emerald-800' : 'rounded-full bg-amber-100 px-2 py-1 font-semibold text-amber-800'}>{corridor.priority}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="intervention-assessment-heading">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Intervention Assessment</p>
              <h2 id="intervention-assessment-heading" className="mt-1 text-2xl font-black">{pasadenaDemoCommuter.id} · Eagle Rock → Pasadena</h2>
              <p className="mt-1 text-sm text-slate-600">Baseline: gasoline single-occupancy commute · Tue/Wed/Thu · generalized zones only</p>
            </div>
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-2xl bg-slate-950 p-5 text-white">
              ...
              <div className="mt-4 border-t border-white/10 pt-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Why this appeared</p>
                <ul className="mt-2 space-y-2 text-sm text-slate-200">
                  {pasadenaInterventionWhy.map(reason => <li key={reason}>• {reason}</li>)}
                </ul>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <MiniStat label="Baseline SOV" value={`${pasadenaNetVmtAssessment.baselineSovMiles?.toFixed(1)} mi`} />
                <MiniStat label="EV detour" value={`${pasadenaNetVmtAssessment.incrementalEvDetourMiles?.toFixed(1)} mi`} />
                <MiniStat label="Net VMT" value={`${pasadenaNetVmtAssessment.modeledNetVmtAvoided?.toFixed(1)} mi`} />
              </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="w-full min-w-[760px] text-left text-xs">
              <thead className="text-slate-500"><tr className="border-b border-slate-200"><th className="pb-2">Corridor</th><th className="pb-2">Gas SOV</th><th className="pb-2">Transit strong</th><th className="pb-2">EV-route strong</th><th className="pb-2">Both credible</th><th className="pb-2">Unsupported</th><th className="pb-2">Modeled net VMT</th></tr></thead>
              <tbody>{pasadenaCorridorInterventionSummary.map(row => <tr key={row.corridor} className="border-b border-slate-100"><td className="py-3 font-semibold">{row.corridor}</td><td>{row.gasolineSov}</td><td>{row.transitStrong}</td><td>{row.evRouteStrong}</td><td>{row.bothCredible}</td><td>{row.unsupported}</td><td>{row.modeledNetVmtOpportunityMiles.toFixed(1)} mi</td></tr>)}</tbody>
            </table>
          </div>
          <p className="mt-3 text-xs leading-5 text-slate-500">These corridor aggregates are synthetic scenario fixtures for interface testing, not empirical Pasadena findings or forecasts.</p>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          ...
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2"><Users size={20} className="text-emerald-700" /><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Selected Decision Card</p><h2 className="font-bold">{activeCorridor.name}</h2></div></div>
        ...
        <footer className="rounded-2xl border border-slate-200 bg-white p-4 text-xs leading-5 text-slate-600">
          <strong>Research-beta guardrail:</strong> {pasadenaDemoMeta.disclaimer} No live dispatch, route activation, guaranteed transportation, or certified environmental outcome is represented here.
        </footer>
      </main>
    </div>
  );
...
function InterventionCard({ icon, title, result }: { icon: React.ReactNode; title: string; result: InterventionResult }) {
  const scoreLabel = result.score === null ? '—' : `${result.score} / 100`;
  return (
    <article className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      <div className="flex items-start justify-between gap-3"><span className="rounded-xl bg-white p-2 text-emerald-700 shadow-sm">{icon}</span><EvidenceBadge state={result.evidenceState} /></div>
      <h3 className="mt-3 font-bold">{title}</h3>
      <div className="mt-2 flex items-end justify-between gap-2"><span className="text-xs font-bold uppercase text-slate-500">{result.strength}</span><strong className="text-xl">{scoreLabel}</strong></div>
      <p className="mt-2 text-xs leading-5 text-slate-600">{result.eligibilityStatus === 'eligible' ? result.reasons[0] : result.limitingFactors[0]}</p>
    </article>
  );
}
