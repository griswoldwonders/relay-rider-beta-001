...
export const WalletAdminScreen: React.FC<Props> = ({ onBack }) => {
  ...
  return <main className="green-wallet-admin">
    ...
    <div className="container green-wallet-admin__shell">
      <div className="green-wallet-admin__intro"><div><span className="green-wallet-kicker">Administrative queue</span><h1>Review redemption requests</h1><p>Confirm program eligibility before recording a manual program decision. Amounts are shown as program benefits; this view is a prototype and does not settle a charging session.</p></div><div className="green-wallet-admin__count"><strong>{requests.filter(request => request.status === 'requested' || request.status === 'under-review').length}</strong><span>open requests</span></div></div>
      {requests.length === 0 ? <div className="green-wallet-admin__empty"><ShieldCheck size={22} /><strong>No requests yet</strong><span>Participant requests will appear here for review.</span></div> : <div className="green-wallet-admin__list">{requests.map(request => <article className="green-wallet-admin__row" key={request.id}><div className="green-wallet-admin__request"><div><span className="green-wallet-kicker">{request.id}</span><strong>{request.requestedUnits} Green Route Credits</strong></div><span>{hubById[request.chargingHubId]?.name ?? request.chargingHubId} · Submitted {new Date(request.requestedAt).toLocaleString()}</span><small>Status: {request.status === 'requested' ? 'Pending review' : request.status === 'fulfilled' ? 'Manual program decision recorded' : 'Denied'}</small>{request.reviewNote && <small>Note: {request.reviewNote}</small>}</div>{(request.status === 'requested' || request.status === 'under-review') && <div className="green-wallet-admin__actions"><button type="button" className="green-wallet-admin__approve" onClick={() => reviewRedemptionRequest(request.id, 'fulfilled', 'Manual program decision recorded by administrator.')}><CheckCircle2 size={16} /> Record manual decision</button><button type="button" className="green-wallet-admin__deny" onClick={() => reviewRedemptionRequest(request.id, 'denied', 'Denied during pilot review.')}><XCircle size={16} /> Deny</button></div>}</article>)}</div>}
      <div className="green-wallet-disclosure"><ShieldCheck size={15} /><span><strong>Operational boundary:</strong> a fulfilled request records a manual program decision. It does not reserve a charger, start a charging session, or process a payment.</span></div>
    </div>
  </main>;
};
