...
class RelayRiderPolicyTests(unittest.TestCase):
    ...
    def test_blocks_force_push_variants_and_refspecs(self) -> None:
        for command in (
            ...
            with self.subTest(command=command):
                ...
                self.assertIn("force", block_message(result).lower())
    ...
    def test_blocks_git_push_without_an_explicit_approved_destination(self) -> None:
        for command in ("git push", "git -C . push origin HEAD:main"):
            with self.subTest(command=command):
                result = run_policy(tool_name="terminal", tool_input={"command": command})
                self.assertIn("development branch", block_message(result).lower())

    def test_allows_push_with_git_c_option_to_explicit_development_branch(self) -> None:
        result = run_policy(
            tool_name="terminal",
    ...
    def test_blocks_terminal_worktree_escape_and_malformed_command(self) -> None:
        for command in ("printf x > ../outside.txt", "git -C ../other push origin hermes/example", None):
            with self.subTest(command=command):
                result = run_policy(tool_name="terminal", tool_input={"command": command})
                self.assertIn("worktree" if command else "validate", block_message(result).lower())
